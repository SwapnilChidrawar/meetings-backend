...
const editMeeting = (req, res, next) => {
  ...
  if (!data?.isError && data.length) {
    ...
    if (index === -1) {
      ...
      let meetingIndex = parsedData[index].meetings.findIndex((meeting) => {
        meeting.meetingId === req.body.meetingId;
      });

      parsedData[index]["meetings"][meetingIndex].date = req.body.date;
      parsedData[index]["meetings"][meetingIndex].time = req.body.time;
      parsedData[index]["meetings"][meetingIndex].desc = req.body.desc;
      parsedData[index]["meetings"][meetingIndex].participant =
